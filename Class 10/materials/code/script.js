...
Manager.addSubordinate = function (employee) {
  this.subordinates.push(employee);
  console.log(`${employee.name} is now a subordinate of ${this.name}.`);
};

let Director = Object.create(Manager);
Director.makeDecision = function () {
  console.log(`${this.name} is making a decision for the company.`);
};

let john = Object.create(Employee);
john.init('John', 'Developer', 3000);
john.work(); // John is working as a Developer
john.getSalary(); // John earns 3000 per month.

let jane = Object.create(Manager);
jane.init('Jane', 'Project Manager', 5000, 'IT');
jane.work(); // Jane is working as a Project Manager
jane.getSalary(); // Jane earns 5000 per month.
jane.manage(); // Jane is managing the IT department.
jane.addSubordinate(john); // John is now a subordinate of Jane.

let bob = Object.create(Director);
bob.init('Bob', 'Director', 10000, 'Company');
bob.work(); // Bob is working as a Director
bob.getSalary(); // Bob earns 10000 per month.
bob.manage(); // Bob is managing the Company department.
bob.makeDecision(); // Bob is making a decision for the company.
bob.addSubordinate(jane); // Jane is now a subordinate of Bob.
